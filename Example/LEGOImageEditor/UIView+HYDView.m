...
CGAffineTransform YDmakeTransform(CGFloat xScale, CGFloat yScale, CGFloat theta, CGFloat tx, CGFloat ty)
{
...
- (CGAffineTransform) ydtransform{
    return self.transform;
}

- (void)setYdtransform:(CGAffineTransform)ydtransform {
    self.transform = ydtransform;
}
...
{
    self.transform = YDmakeTransform(xScale, self.yscale, self.rotation, self.tx, self.ty);
}
...
{
    self.transform = YDmakeTransform(self.xscale, yScale, self.rotation, self.tx, self.ty);
}
...
{
    self.transform = YDmakeTransform(self.xscale, self.yscale, theta, self.tx, self.ty);
}
...
{
    self.transform = YDmakeTransform(self.xscale, self.yscale, self.rotation, tx, self.ty);
}
...
{
    self.transform = YDmakeTransform(self.xscale, self.yscale, self.rotation, self.tx, ty);
}
...
- (CGPoint) pointInTransformedView: (CGPoint) aPoint
{
    ...
    CGPoint updatedItem = CGPointApplyAffineTransform(offsetItem, self.transform);// 某点通过矩阵变换之后的点
